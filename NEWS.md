...
- genomicSimulation now stores chromosome names in full. These can be seen in the output of `see.genetic.map`.
- Added functions `change.eff.set.centres` and `change.eff.set.centres.of.allele`, which allow the user to define centring values that will be subtracted from the breeding value contribution of alleles at a particular marker. `change.eff.set.centres.of.allele`, in particular, is designed for GBLUP marker effects, which should have an allele-frequency factor subtracted from allele counts before they are multiplied to marker effects to get the estimated breeding value.
- Added functions `create.markerblocks` and `create.markerblocks.from.chrsplit`, as well as the corresponding `see.markerblocks`.
- Added functions `see.local.GEBVs` and `save.local.GEBVs`, which take as input a MarkerBlocks object created by `create.markerblocks` or similar. The old local GEBV functions `save.local.GEBVs.blocks.from.file` and `save.local.GEBVs.blocks.from.chrsplit` are still functional, but now use `see.local.GEBVs` under the hood.
- Added function `see.GEBVs`, for symmetry with `see.local.GEBVs`. It is a simple wrapper around `see.group.data`.
...
- Functions that accept `map1`/`map2` and `cap1`/`cap2` parameters now also accept `map`, if neither `map1` nor `map2` are set, and `cap`, if neither `cap1` or `cap2` are set.
