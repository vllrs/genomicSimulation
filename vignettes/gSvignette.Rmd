...
[A formal summary of this tool's design and performance is available in research paper format. Some information (capabilities, function names) may be out of date.](http://dx.doi.org/10.1093/g3journal/jkac216)
...
This vignette will go through the details of genomicSimulation simulations, and then show some simple simulation scripts. An alternative way to learn about the package is through the [Features and Template Guide (in the genomicSimulationC documentation)](https://vllrs.github.io/genomicSimulationC/html/templates.html), which works through a larger list of examples of breeding program simulation components
...
![Boxplot of set-up time benchmark results](benchmark_gS_v0.2.6_loading.png)
...
![Boxplot of simulating crossing benchmark results](benchmark_gS_v0.2.6_crossing.png)
...
**Min:** 0.00284 seconds 
**Mean:** 0.00300 seconds 
**Max:** 0.00529 seconds 

![Boxplot of calculating breeding value benchmark results](benchmark_gS_v0.2.6_bvcalc.png)
...
Generally, these default parameter settings should be satisfactory. If you do need to change them, add a line of the format `#define CONTIG_WIDTH 2048` (with `CONTIG_WIDTH` and `2048` replaced with your desired values) to the first line of `src/genomicSimulation-init.c`, then rebuilding the package.

The following type aliases store different types of identifiers in genomicSimulation's internal library. They default to being `unsigned int`, a type which commonly is capable of storing integers between 0 and 4,294,967,295. 

- GSC_ID_T is used for all unique identifiers in genomicSimulation (group numbers, pedigree IDs, genetic map IDs, and marker effect set IDs).
- GSC_GENOLEN_T is used to store the total number of genetic markers tracked by the simulation.
- GSC_GLOBALX_T is used to store the total number of genotypes in the simulation.
- GSC_LOCALX_T only needs to hold values between 0 and CONTIG_WIDTH. It will be automatically resized if you resize CONTIG_WIDTH, so it is unlikely you would need to redefine this type.

If you need to generate or store over 4,294,967,294 genotypes or genetic markers in a single simulation, you can redefine one or more of the type aliases to a larger type by adding lines of the format `#define GSC_ID_T long unsigned int` (with `GSC_ID_T` and `long unsigned int` replaced with your desired values) to the first line of `src/genomicSimulation-init.c`, then rebuilding the package.
