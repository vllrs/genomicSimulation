---
title: "genomicSimulation (v0.2.4) Vignette"
...
output:
  rmarkdown::html_vignette:
      ...
      #theme: simplex
vignette: >
  %\VignetteIndexEntry{genomicSimulation (v0.2.4) Vignette}
...
**Significant changes in v0.2.4**:

- Multiple sets of marker effects can now be loaded at a time.
- If load.data is called with an effect.file, then its return type will be a list with two elements (\$groupNum, the group number of the founder group, and \$effectID, the identifier of the set of effects loaded from the effect file) instead of only the group number. Scripts that use a line like `group0 <- load.data(...)` might need updating to `init <- load.data(...); group0 <- init$groupNum`. (or to `load.data(...); group0 <- 1L`, since both of the numbers in the list will be exactly 1.)
...
- `[effect set] <- load.different.effects(effect.file)`
...
- `[selected.group] <- select.by.gebv(from.group, low.score.best = FALSE, percentage OR number, eff.set=1L)`
...
- `[float: the minimum possible GEBV] <- see.minimum.GEBV(eff.set=1L)`
