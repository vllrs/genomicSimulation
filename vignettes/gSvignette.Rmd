---
title: "genomicSimulation (v0.2.2.4) Vignette"
...
The `SimData` object saves names of markers as-is, as long as the name is under 30 characters long. Behaviour is undefined if any marker name is longer than 30 characters. (The currently-non-modifiable NAME_LENGTH setting (see Indexes: Global Settings Index) is associated with work on fixing this limitation).

Becauses the `SimData` stores its markers in genome order, any outputs produced will have the markers ordered according to the genetic map loaded at its creation. If an output method provides genotypes as strings of alleles, the string will also be ordered according ot this genetic map.
...
Only one 'set' of effect values can be loaded at a time, but effect files can be swapped out at any time using `load.different.effects()`.
...
Every function except those that pull data into R (functions whose names begin with `see.`) and the special case of `make.label` (see following section: it returns one integer but that integer does not represent a group number) return either one integer or a vector of integers. If the integer is 0, it means the function succeeded but it is not a function that acts on groups. Otherwise, the return value is the group number(s) of the group(s) that were created. This means that in your R environment, the groups you are working with will be to all intents and purposes just integers. 

Note that only genomicSimulation functions can actually make changes to the groups. Modifying the numbers returned by genomicSimulation in the R environment will have no effect, and furthermore genomicSimulation will not, for example, delete a number in the R environment if that group ceases to exist.

## Understanding Custom Labels

Sometimes you might have reason to want to track extra details about your genotypes (a common example is the age of the simulated individual), or you might want a categorisation system that persists even when groups are broken apart, dissolved, and/or reformed in efforts to perform particular selection or mating procedure.
...
Also like groups, use `delete.label` to delete a custom labe (all simulated individuals will still exist, but they will no longer have any value under the custom label that no longer exists). Merely removing the label ID from the R environment does nothing to remove the label from simulation memory.

## Understanding Identifiers
There are four categories of identifiers that may be observed during use of the simulation.

1. Group numbers, which were discussed in the preceding section on groups. They are integers greater than 0 that define a set of genotypes/lines. Any number of lines can share a group number, and a line's group number may be changed over the course of the simulation.
1. Label identifiers, which were discussed in the preceding section on labels. Like group numbers, they are integers greater than 0. They define a particular custom label. A label keeps its same identifier for its lifetime.  
1. IDs. These are integers greater than 0 that are unique to a particular genotype for the lifetime of the simulation. IDs are used for tracking pedigree. However, genotypes are not guaranteed to have an ID: genotypes created during crossing with the setting `give.ids` set to FALSE will not be allocated IDs and will have 0 instead. 
...
```{R}
# Starting to use genomicSimulation: load the small initial data set
cows <- load.data("../tests/testthat/helper_genotypes.txt",
                "../tests/testthat/helper_map.txt",
                "../tests/testthat/helper_eff.txt")
...
f1 <- cross.randomly.between(cows, bulls, n.crosses=5)
...
calvesF <- make.group.from.label(sexLabel, FEMALE, group=f1)
calvesM <- f1 #the calves left in F1 must be all the males

#... and so on
```

More sample scripts for simulating common scenarios using genomicSimulation can be found idevn [this Templates page](https://vllrs.github.io/genomicSimulationC/html/templates.html).
...
- NAME_LENGTH (default 45). This parameter is not yet modifiable in the R version
...
- `[group] <- cross.randomly.between(group1,group2,cap1,cap2,n.crosses,[[settings]])`
...
- `[group] <- make.group.from.label(label, value, in group(s)/no group)`
- `[group] <- make.group.from.label.range(label, rangeLowEnd, rangeHighEnd, in group(s)/no group)`
...
- `[string containing best allele at each SNP] <- see.optimal.haplotype()`
- `[string containing best allele that exists in the group at each SNP] <- see.optimal.possible.haplotype()`
- `[float: the maximum possible GEBV] <- see.optimal.GEBV()`
- `[float: the maximum possible GEBV from the pool of alleles that exist in the group] <- see.optimal.possible.GEBV()`
