---
title: "genomicSimulation (v0.1-5) Vignette"
...
# Introduction
This package can be used to create 'digital twins' of breeding schemes. It is a state-based simulation tool that offers scripting functions to simulate crossing and genomic selection on SNP markers. It was created with self-pollinated plant species in mind. 

The package itself is an interface to a set of C functions. Using C to perform the computation-intensive parts of simulation allows for quick runtimes. 
...
The package's functionality is state-based - that is, simulation is dependent on the current set of working data, which is stored in a C-level struct called `SimData`. An external pointer to this struct is saved in a package-global R variable called `sim.data$p`. `sim.data$p` is hidden during regular use. 
...
### Effects
The simulation cannot run without having a genetic map for the SNPs it tracks. However, it can run with no effect values loaded, so in cases where breeding values are not to be used, there is no need to provide an effect file.
...
A line can only belong to one group, so functions that overwrite allocations (eg `make.group`) effectively pull lines out of their old group and put them in a new one. This ensures that lines can only be deleted once.

Group numbers are natural numbers starting at 1. Group numbers are not guaranteed to be allocated sequentially, because some crossing functions may create and delete temporary groups to carry out their task, which could result in the final group number they return being out of order.
...
The only inbuilt selection method in genomicSimulation is `select.by.gebv`. However, custom selection methods can be written in R thanks to the `make.group` function, which takes an arbitrary set of indices and moves them into a new group. Indices of genotypes can be accessed via `see.group.gebvs` or `see.group.data` with data type parameter `I`, operated on, then passed back to `make.group` to perform selection.
...
## Writing Custom Crossing Plans in R
Crossing plans refers to interactions between identities of genotypes and how you select for them. Making custom crossing plans will be a matter of splitting groups in order to perform finer-grained selection or further crossing. Because the range of possible crossing plans this guide cannot offer a solution but only some suggested starting points.

Ideas for TODO the example:
- Maintaining families: select some good ones from random crossing. Self (families) and pick the best from each row. Continue selfing with that one. This is pedigree selection.
- Half-sib families, and picking the best from each??
...
# Simulate crosses
f1 <- cross.randomly(g0, n.crosses=50)
f1.selected <- select.by.gebv(f1, number=10)
delete.group(f1)

f2 <- self.n.times(f1.selected, 1, offspring=15)
f2.families <- break.group.into.families(f2)
for (family in f2.families) {
  #something
}
